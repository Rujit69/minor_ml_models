...
def train_model(model, dataloader, criterion, optimizer, device, num_epochs=10, checkpoint_path='model4.pth', best_model_path='best_model.pth', history_path='training_history.pth'):
    start_epoch = 0
    train_losses = []
    train_accuracies = []
    best_accuracy = 0.0
    ...
    # Plot training loss
    plt.subplot(1, 2, 1)
    plt.plot(range(len(train_losses) + 1), [0] + train_losses, label='Training Loss')  # Include 0 and all epochs
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(len(train_accuracies) + 1), [0] + train_accuracies, label='Training Accuracy')  # Include 0 and all epochs
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
...
if __name__ == '__main__':
    ...
    train_model(model, train_loader, criterion, optimizer, device, num_epochs, checkpoint_path, best_model_path, history_path)